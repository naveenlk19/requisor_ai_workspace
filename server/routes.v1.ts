  ...
  app.post("/api/feature-candidates", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (incomingSource === "manual") {
        ...
        const { featureTitle, whyNow, sourceContext, projectId } = parsed.data;
        if (projectId) {
          const project = await storage.getProject(projectId);
...
Sort the array by riceScore descending (best first).`;
  ...
  app.patch("/api/feature-candidates/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const { featureTitle, whyNow, evidence, uiChanges, dataModelChanges, workflowChanges, tasks, lastSentToAgent, lastSentAt, sourceContext, projectId } = req.body;
      ...
      if (projectId !== undefined) {
        ...
        updates.projectId = projectId === null ? null : Number(projectId);
      }
